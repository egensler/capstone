...
def get_pie_chart(site_dropdown):
    ...
    if site_dropdown == 'ALL':
        ...
        fig = px.pie(filtered_df, #values='class',
                    names='class',
                    title = 'Successful count for ' + site_dropdown)
        return fig
...
def get_scatter_chart(site_dropdown, payload_slider):
    ...
    mask=spacex_df['Payload Mass (kg)'].between(min_payload,max_payload)
    filtered_df = spacex_df[mask]
    if site_dropdown == 'ALL':
        fig = px.scatter(filtered_df, x= 'Payload Mass (kg)', y='class', color="Booster Version Category", #values='class', #filtered_df['class']
            title='All Launches by payload to class')
        return fig
    else:
        
        filtered_df = filtered_df.loc[filtered_df['Launch Site'] == site_dropdown]
        fig = px.scatter(filtered_df, x= 'Payload Mass (kg)', y='class', color="Booster Version Category",
            title='Launches by payload to class for ' + site_dropdown)
        return fig
# Run the app
